...
export class ExtrefApplicantInformationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.refApplicantForm.addControl('addressVO', this.fb.group({
      addressFormatCd: [''],
      addressLine1: ['', [Validators.required, Validators.maxLength(100), this.customValidator.addressAndCityValidator()]],
      addressLine2: ['', [Validators.maxLength(50), this.customValidator.addressAndCityValidator()]],
      stateCd: ['', [Validators.required]],
  ...
  getchildFormGroup() { return (<FormGroup>(this.refApplicantForm.controls.addressVO)).controls }
  ...
  onSsnOptionChange(checkChange:MatCheckboxChange){
  if(checkChange.checked){
    this.getFormData().ssn.clearValidators();
  }
  else{
    this.getFormData().ssn.setValidators([Validators.required, this.customValidator.ssnValidator()]);
  }
  //this.getFormData().ssn.updateValueAndValidity();
  }
