...
export class ExtrefContactInformationComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.refContactForm = this.fb.group({
      ...
      prefLangTypeCd: ['', Validators.required],
    });

    this.extRefService.stepReady(this.refContactForm,"three");
    this.extRefService.sharedParam.subscribe(param=>{this.age = param;
      if (this.age >= 18) {
        this.ageSW = true;
        ...
        this.getFormData().designeeSw.updateValueAndValidity();
        this.refContactForm.get('dsgnFirstName').setValidators(Validators.required);
        this.refContactForm.get('dsgnLastName').setValidators(Validators.required);
  ...
  //  const ages = this.refService.getAge();
  //  console.log(ages);
  //  console.log(this.age);
    // if (this.age >= 18) {
    //   this.ageSW = true;
  ...
  onPhoneType(value: string) {
    ...
    if (value === 'cellPhone') {
      cellPhNum.setValidators([Validators.required,Validators.maxLength(10),this.customValidator.phonenumberValidator(),]);
  ...
  onCurrentLivingArrangement(event) {
    ...
    } else if (event.target.value === 'NURS' || event.target.value === 'ICF') {
    ...
    else if(event.target.value === 'OWNH'){
